...
def load_model():
    global rf_model, le_commodity, le_district

    if rf_model is None:

        model_path = hf_hub_download(
            repo_id="Aruljothi1507/pure-harvest-ai",
            filename="tn_final_model.pkl"
        )

        commodity_path = hf_hub_download(
            repo_id="Aruljothi1507/pure-harvest-ai",
            filename="le_commodity.pkl"
        )

        district_path = hf_hub_download(
            repo_id="Aruljothi1507/pure-harvest-ai",
            filename="le_district.pkl"
        )

        with open(model_path, "rb") as f:
            rf_model = pickle.load(f)

        with open(commodity_path, "rb") as f:
...
def load_pincodes():
    ...
    if pincodes is None:
        ...
        geo_path = hf_hub_download(
            repo_id="Aruljothi1507/pure-harvest-ai",
            filename="pincode.geojson"
        )

        pincodes = gpd.read_file(geo_path)
